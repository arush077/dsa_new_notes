...
// condition on amount : if amount == 0 then return 0
    ...
    int no(int i, int amount,vector<int>& coins,vector<vector<int>> &dp){
        ...
        int take = 1e9;
        if(amount >= coins[i]){
        take = 1 + no(i,amount-coins[i],coins,dp);
        }
        int nottake = 0 + no(i+1,amount,coins,dp);
        return dp[i][amount] = min(take,nottake);
    }

    int coinChange(vector<int>& coins, int amount) {
        vector<vector<int>> dp(12,vector<int>(amount+1,-1));
        int call =  no(0,amount,coins,dp);
        if(call>=1e9){return -1;}
        return call;
    }
